...
def optimize(motion_class, points, v_max, v_min, a_max, a_min, prediction_horizon, X0, t_bound, motion_model, forward_kinematic, N=100, d_tol=0.01, constrain_final_point=False, cpc_tolerance=0.00001, obstacles=[]):
    ...
    x_dyn = ca.MX.sym('x_dyn', 2*len(v_max)) # x_dyn looks like [x0, x1, v0, v1], column vector
    x_dyn_num_cols = x_dyn.size()[0]
    state_dim = len(v_max)

    control = ca.MX.sym('control', len(v_max)) # control looks like [u0, u1], column vector
    control_num_cols = control.size()[0]
    lambda_num_cols = prediction_horizon #assume starting point is not waypoint
    x_sample = ca.MX.sym('x_sample', 3*len(v_max) + 3*(lambda_num_cols)) # x_sample looks like [x_dyn, u0, u1, lamdas, mus, vs]

    X = ca.MX.sym('X', 3*len(v_max) + 3*(lambda_num_cols), N) # X looks like [[x_0], ... , [x_N]] where x_i = [x_i, v_i, u_i, lambda_i, mu_i, v_i] 
    tn = ca.MX.sym('tn')
    dt = tn/(N)
    P = ca.MX.sym('P', 2*len(v_max)+len(points[0])+1) # P looks like [x_0, v_0, u_0, lambda_0, mu_0, v_0]
    ...
    for i in range(1,prediction_horizon+1):
        if i==len(points):
            ...
            point = points[i]
        # print('point:', point)
        norms_array.append(ca.norm_2(forward_kinematic(x_dyn[:state_dim]) - ca.vertcat(point[0], point[1]))**2)
    ...
    xf = ca.vertcat(points[prediction_horizon][0], points[prediction_horizon][1])
    for k in range(N): # N is the number of columns
        ...
        cur_lbx[u_start_index:lambda_start_index] = ca.horzcat(*a_min)
        cur_ubx[u_start_index:lambda_start_index] = ca.horzcat(*a_max)
        if k==0:
            # Initial state
            ...
            # Motion model
            g.append(X[:u_start_index, k] -  X[:u_start_index, k-1] - dt*motion_model(X[:u_start_index, k-1], X[u_start_index:lambda_start_index, k-1]))
            lbg.append(ca.DM.zeros(x_dyn_num_cols))
            ubg.append(ca.DM.zeros(x_dyn_num_cols))
            # lambda constraints
            if k == N-1: #last point

...
def inverse_kynematics_dynamic(self, current_end_effector, current_params, future_goals, k =1):
    ...
    theta1, theta2 = self.converge_to_goal(current_params, future_goals[0], t=0.1)
...
def new_pose_luoponov_control(self, current_params, goal, t=0.1, max_iter=100):
    ...
    while np.linalg.norm(e) > 0.01 and count < max_iter:
        ...
        theta1 += q_dot[0, 0]*t
        theta2 += q_dot[1, 0]*t
        e, J = self.compute_error((theta1, theta2), goal)
        count += 1
    if count == max_iter:
        return None, None
    return theta1, theta2

def converge_to_goal(self, current_params, goal, t=1):
    count =0
    theta1, theta2 = current_params
    cur_pose = (theta1, theta2)
    theta1, theta2 = self.new_pose_luoponov_control(cur_pose, goal, t)
    if theta1 != None:
        return theta1, theta2
    else:
        # throw error
        print('Error, could not converge to goal')
        return current_params


def compute_error(self, current_params, future_goal):
    ''' Compute the error for the inverse kinematics. '''
    x1, y1, x2, y2, x3, y3 = self.compute_link_positions(current_params[0], current_params[1])
    # Compute Jacobian
    J = self.compute_jacobian(current_params[0], current_params[1])
...
def compute_positions(self, starting_configurations, intermediate_points):
    ...
    waypoints_configs = [starting_configurations]
    theta1, theta2 = starting_configurations
    for i in range(1,len(intermediate_points)):
        configurations.append((theta1, theta2))
        theta1, theta2 = self.inverse_kynematics_dynamic(intermediate_points[i-1],(theta1, theta2), intermediate_points[i:])
        if intermediate_points[i][5] == 1:
            waypoints_configs.append((theta1, theta2))

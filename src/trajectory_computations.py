import sys
from .analysis import analyze_single_trajectory, remove_trajectory
...
def compute_single_trajectory(start_point, end_point, a_max, a_min, v_max, v_min, starting_velocity, ending_velocity, analyze, remove_bad):
    ...
    for i in range(len(results)):
        if i != max_index:
            extended_trajectory = compute_alpha(start_point[i], starting_velocity[i], end_point[i], ending_velocity[i], a_max[i], a_min[i], v_max[i], v_min[i], tf)
    ...
    if analyze:
        analyze_single_trajectory(solution, start_point, end_point, starting_velocity, ending_velocity)
    if remove_bad and remove_trajectory(solution, start_point, end_point, starting_velocity, ending_velocity):
        return None
    return solution

def compute_trajectory_set(points, a_max, a_min, v_max, v_min, starting_velocity_list, ending_velocity_list, df, start_index, end_index, prediction_horizon, analyze, remove_bad):
    ...
    for i, v0 in enumerate(starting_velocity_list):
        ...
        for j, vf in enumerate(ending_velocity_list):
            params = compute_single_trajectory(start_point, end_point, a_max, a_min, v_max, v_min, v0, vf, analyze, remove_bad)
            if params == None:
                continue
...
def compute_all_trajectories(points, velocity_df, max_acc, min_acc, max_vel, min_vel, prediction_horizon=None, analyze=False, remove_bad=False):
    ...
    for i in range(prediction_horizon):
        ...
        trajectories, dict_res = compute_trajectory_set(points, max_acc, min_acc, max_vel, min_vel, start_vel, end_vel, dict_res, i, end_point, prediction_horizon, analyze, remove_bad)
...
def compute_trajectory_set_multi_pose(dict_poses, start_level, end_level, start_node, end_node,  a_max, a_min, v_max, v_min, starting_velocity_list, ending_velocity_list, result_dict, prediction_horizon, analyze, remove_bad):
    ''' Given two points, compute all possible trajectories between the two points using the provided velocity lists. Assemble the results in the dictionary used for the shortest path algorithm. 
    In such dictionary, all the intermediate graph nodes with the same starting coordinates but different starting velocities are cosidered as different states'''
    ...
    for i, v0 in enumerate(starting_velocity_list):
        ...
        for j, vf in enumerate(ending_velocity_list):
            params = compute_single_trajectory(start_pose, end_pose, a_max, a_min, v_max, v_min, v0, vf, analyze, remove_bad)
            if params == None:
                continue
...
def compute_all_trajectories_multi_pose(dict_poses, velocity_df, a_max, a_min, v_max, v_min, prediction_horizon=None, analyze=False, remove_bad=False):
    ...
    for level in range(prediction_horizon):
        for start_node in range(len(dict_poses[level])):
            ...
            for end_node in range(len(dict_poses[next_level])):
                ...
                trajectories, result_dict = compute_trajectory_set_multi_pose(dict_poses, level, next_level, start_node, end_node, a_max, a_min, v_max, v_min, starting_velocity_list, ending_velocity_list, result_dict, prediction_horizon, analyze, remove_bad)
                results.extend(trajectories)
